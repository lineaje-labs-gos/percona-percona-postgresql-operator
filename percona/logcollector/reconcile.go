...
func wireSidecars(cr *v2.PerconaPGCluster) error {
	...
	if !hasDedicatedRepoHost(cr) {
		return nil
	}

	repoContainers, err := repoHostContainers(cr)
	if err != nil {
		return errors.Wrap(err, "build repo host containers")
	}
	if cr.Spec.Backups.PGBackRest.RepoHost == nil {
		cr.Spec.Backups.PGBackRest.RepoHost = &v1beta1.PGBackRestRepoHost{}
	}
	repoHost := cr.Spec.Backups.PGBackRest.RepoHost
	repoHost.Sidecars = append(repoHost.Sidecars, repoContainers...)
	repoHost.SidecarVolumes = append(repoHost.SidecarVolumes, volumes...)
	return nil
}
...
// hasDedicatedRepoHost reports whether the cluster provisions a dedicated
// pgBackRest repo host pod, which happens when at least one repo is PVC-backed.
func hasDedicatedRepoHost(cr *v2.PerconaPGCluster) bool {
